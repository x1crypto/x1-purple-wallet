...
static CBlock CreateXDSGenesisBlock(uint32_t nTime, uint32_t nNonce, uint32_t nBits, int32_t nVersion)
{
    CMutableTransaction txNew;
    txNew.nVersion = 1;
    txNew.vin.resize(1);
    ...
    unsigned char scriptSigBytes[47] = {0x0, 0x1, 0x2a, 0x2b, 0x68, 0x74, 0x74, 0x70, 0x73, 0x3a, 0x2f, 0x2f, 0x77, 0x77, 0x77, 0x2e, 0x62, 0x6c, 0x6f, 0x63, 0x6b, 0x63, 0x68, 0x61, 0x69, 0x6e, 0x2e, 0x63, 0x6f, 0x6d, 0x2f, 0x62, 0x74, 0x63, 0x2f, 0x62, 0x6c, 0x6f, 0x63, 0x6b, 0x2f, 0x36, 0x31, 0x31, 0x30, 0x30, 0x30};

    txNew.vin[0].scriptSig = CScript(&scriptSigBytes[0], &scriptSigBytes[47]);
...
public:
    CMainParams()
    {
        ...
        consensus.nMinerConfirmationWindow = 2016;       // bitcoin: nPowTargetTimespan / nPowTargetSpacing, we can't use that, would be 1
        ...
        consensus.TenMinutesSpacingHeight = 165740; // nPowTargetSpacing changes to 600 seconds
        ...
        vFixedSeeds = std::vector<SeedSpec6>({
                                              SeedSpec6({{0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0xff, 0xff, 0x86, 0x7a, 0x59, 0x98}, 23333}), // 134.122.89.152
                                              SeedSpec6({{0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0xff, 0xff, 0xa1, 0x23, 0x9c, 0x60}, 23333}), // 161.35.156.96
                                              SeedSpec6({{0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0xff, 0xff, 0x2e, 0x11, 0x61, 0x24}, 23333})  // 46.17.97.36
        });
...
public:
    CTestNetParams()
    {
        ...
        consensus.fPowPosNoRetargeting = false;
        consensus.nRuleChangeActivationThreshold = 1512; // 75% of 2016
        consensus.nMinerConfirmationWindow = 338;       // bitcoin: nPowTargetTimespan / nPowTargetSpacing
        consensus.vDeployments[Consensus::DEPLOYMENT_TESTDUMMY].bit = 28;
        consensus.vDeployments[Consensus::DEPLOYMENT_TESTDUMMY].nStartTime = 1199145601; // January 1, 2008 (any past value should work)
        consensus.vDeployments[Consensus::DEPLOYMENT_TESTDUMMY].nTimeout = 1230767999;   // December 31, 2008 (any past value should work)
        ...
        nDefaultPort = 38333 + 1;
        nPruneAfterHeight = 100000;
        m_assumed_blockchain_size = 320;
        m_assumed_chain_state_size = 4;

        genesis = CreateXDSGenesisBlock(1606692960, 24270024, 0x1e0fffff, 1);
        assert(genesis.hashMerkleRoot == uint256S("0xe3c549956232f0878414d765e83c3f9b1b084b0fa35643ddee62857220ea02b0"));

        const auto powHashGenesisBlock = genesis.GetPoWHash(); // this is the XDS SHA512t PoW hash
        assert(powHashGenesisBlock == uint256S("0x00000d2ff9f3620b5487ed8ec154ce1947fec525e91e6973d1aeae93c53db7a3"));

        consensus.hashGenesisBlock = genesis.GetHash(); // only in case of the genesis block, GetHash() also returns the PoW hash
        assert(consensus.hashGenesisBlock == uint256S("0x00000d2ff9f3620b5487ed8ec154ce1947fec525e91e6973d1aeae93c53db7a3"));
...
public:
    explicit CRegTestParams(const ArgsManager& args)
    {
        ...
        consensus.fPowPosNoRetargeting = true;
