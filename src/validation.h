...
public:
    ...
    CBlockIndex* AddToBlockIndex(const CBlockHeader& block, const uint32_t nPosPow, const uint256 stakeModifierV2) EXCLUSIVE_LOCKS_REQUIRED(cs_main);
    ...
    /**
     * Same as above for proven block headers.
     */
    bool AcceptProvenBlockHeader(
        const CProvenBlockHeader& block,
        BlockValidationState& state,
        const CChainParams& chainparams,
        CBlockIndex** ppindex) EXCLUSIVE_LOCKS_REQUIRED(cs_main);
...
public:
	...
	/**
     * Process incoming proven block headers.
     *
     * May not be called in a
     * validationinterface callback.
     *
     ...
     * @param[out] ppindex If set, the pointer will be set to point to the last new block index object for the given headers
     */
    bool ProcessNewProvenBlockHeaders(const std::vector<CProvenBlockHeader>& block, BlockValidationState& state, const CChainParams& chainparams, const CBlockIndex** ppindex = nullptr) LOCKS_EXCLUDED(cs_main);
